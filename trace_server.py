...
def get_trace_file(filename: str):
    ...
    try:
        ...
        if not file_path.exists():
            return (
                jsonify({"success": False, "error": f"File {filename} not found"}),
                404,
            )
        ...
        return jsonify(
            {
                ...
                "metadata": {
                    ...
                    "total_files_processed": len(
                        [f for f in trace_files if f.get("type") != "error"]
                    ),
                },
            }
        )
...
def search_traces():
    ...
    try:
        ...
        for trace in traces:
            ...
            if run_type:
                has_run_type = any(
                    run.get("run_type") == run_type for run in trace.get("runs", [])
                )
                if not has_run_type:
                    continue
    ...
    try:
        ...
        for trace in traces:
            ...
            if trace.get("start_time") and trace.get("end_time"):
                try:
                    start = datetime.fromisoformat(
                        trace["start_time"].replace("Z", "+00:00")
                    )
                    end = datetime.fromisoformat(
                        trace["end_time"].replace("Z", "+00:00")
                    )
            ...
            for run in runs:
                ...
                stats["run_type_breakdown"][run_type] = (
                    stats["run_type_breakdown"].get(run_type, 0) + 1
                )
        ...
        if think_count > 0:
            stats["structure_analysis"]["think_to_tool_ratio"] = round(
                tool_count / think_count, 2
            )
        if stats["total_traces"] > 0:
            stats["structure_analysis"]["avg_tools_per_trace"] = round(
                tool_count / stats["total_traces"], 2
            )
        ...
        stats["recent_activity"] = [
            {
                ...
                "structure": (
                    "flattened"
                    if not any(
                        run.get("run_type") == "act" for run in trace.get("runs", [])
                    )
                    else "legacy"
                ),
            }
            for trace in sorted(
                traces, key=lambda x: x.get("start_time", ""), reverse=True
            )[:10]
        ]
    ...
    try:
        ...
        if analysis["summary"]["total_traces"] > 0:
            analysis["summary"]["migration_progress"] = round(
                (
                    analysis["summary"]["flattened_traces"]
                    / analysis["summary"]["total_traces"]
                )
                * 100,
                1,
            )
        ...
        if flattened_durations:
            ...
            analysis["performance_comparison"]["flattened"]["total_traces"] = len(
                flattened_durations
            )
        ...
        if legacy_durations:
            ...
            analysis["performance_comparison"]["legacy"]["total_traces"] = len(
                legacy_durations
            )

        if flattened_run_counts:
            analysis["performance_comparison"]["flattened"]["avg_runs_per_trace"] = (
                round(sum(flattened_run_counts) / len(flattened_run_counts), 1)
            )
...
def main():
    ...
    print("🚀 Starting Agent Trace Viewer Server")
    print(f"📁 Traces directory: {TRACES_DIR.absolute()}")
    print(f"🌐 Server URL: http://{args.host}:{args.port}")
    print("📊 API endpoints:")
    print("   - GET /api/traces - List all trace files")
    print("   - GET /api/traces/all - Get all traces")
    print("   - GET /api/traces/<filename> - Get specific trace file")
    print("   - GET /api/search?q=<query> - Search traces")
    print("   - GET /api/stats - Get trace statistics (with flattened insights)")
    print("   - GET /api/structure-analysis - Analyze flattened vs legacy structures")
    print()
