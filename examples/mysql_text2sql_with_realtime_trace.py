...
from myagent.trace import RunStatus
...
class RealTimeTracePrinter:
    ...
    def start_monitoring(self):
        ...
        if RICH_AVAILABLE:
            console.print(
                Panel("🔍 Starting Real-Time Trace Monitor", style="bold green")
            )
        else:
    ...
    def print_trace_start(self, trace):
        ...
        if RICH_AVAILABLE:
            ...
            table.add_row(
                ...
                (
                    trace.request[:50] + "..."
                    if trace.request and len(trace.request) > 50
                    else trace.request or "N/A"
                ),
            )
            ...
            console.print(
                f"  Request: {trace.request[:50] + '...' if trace.request and len(trace.request) > 50 else trace.request or 'N/A'}"
            )
...
class MySQLSchemaTool(BaseTool):
    ...
    async def execute(self, table: str | None = None) -> ToolResult:
        ...
        try:
            ...
            console.print(
                f"🔧 [bold]mysql_schema[/bold] executed: {table or 'all tables'}"
            )
...
class MySQLQueryTool(BaseTool):
    ...
    async def execute(self, sql: str, max_rows: int = 20) -> ToolResult:
        ...
        try:
            ...
            # Print tool execution
            console.print("🔧 [bold]mysql_query[/bold] executed:")
            console.print(f"📝 SQL: {stripped_sql}")
            console.print(
                f"📊 Rows returned: {len(result.split(chr(10))) - 3 if result != 'No rows returned.' else 0}"
            )
            console.print()
...
class MySQLValidateSQLTool(BaseTool):
    ...
    async def execute(self, sql: str) -> ToolResult:
        ...
        try:
            ...
            # Print tool execution
            console.print("🔧 [bold]mysql_validate_sql[/bold] executed:")
            console.print(f"✅ SQL validated: {stripped_sql}")
            console.print("📈 EXPLAIN plan generated successfully")
            console.print()
...
class MonitoringTraceManager(TraceManager):
    ...
    def __init__(self, *args, **kwargs):
        ...
        async def monitored_save_run(run):
            ...
            monitor_task = asyncio.create_task(self._monitor_run(run))
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description="MySQL Text-to-SQL agent example with real-time trace printing"
    )
    parser.add_argument(
        "question",
        nargs="?",
    ...
    if not RICH_AVAILABLE:
        print(
            "Note: Rich library not found. Install with 'pip install rich' for enhanced output."
        )
    ...
    if RICH_AVAILABLE:
        console.print(Panel("🚀 Starting MySQL Text-to-SQL Agent", style="bold blue"))
    ...
    if validate_tool.final_sql:
        ...
        if RICH_AVAILABLE:
            console.print(
                "[yellow]⚠️ No final SQL was validated. Ensure the agent calls mysql_validate_sql once satisfied.[/yellow]"
            )
        else:
            console.print(
...
async def save_traces_to_json(filename: str, question: str) -> None:
    ...
    with open(f"./workdir/traces/{summary_filename}", "w", encoding="utf-8") as f:
        f.write("# MySQL Text-to-SQL Real-time Trace Summary\n\n")
