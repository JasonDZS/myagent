...
class LLMSettings(BaseModel):
    ...
    max_input_tokens: int | None = Field(
        None, description="Maximum number of input tokens for the model"
    )
...
class Settings(BaseModel):
    ...
    workdir: str = Field(
        default="workdir/finance", description="Base working directory"
    )
    ...
    EMBEDDING_MODEL: str = Field(
        default_factory=lambda: os.getenv("EMBEDDING_MODEL", "Pro/BAAI/bge-m3")
    )
    ...
    MARKDOWN_PRESERVE_HTML: bool = Field(
        default_factory=lambda: os.getenv("MARKDOWN_PRESERVE_HTML", "true").lower()
        == "true"
    )
    MARKDOWN_INCLUDE_METADATA: bool = Field(
        default_factory=lambda: os.getenv("MARKDOWN_INCLUDE_METADATA", "true").lower()
        == "true"
    )
    MARKDOWN_FLATTEN_STRUCTURE: bool = Field(
        default_factory=lambda: os.getenv("MARKDOWN_FLATTEN_STRUCTURE", "false").lower()
        == "true"
    )
    ...
    distance: float = Field(
        default=1.5, description="Distance threshold for embedding similarity"
    )
    top_k: int = Field(
        default=5, description="Number of top results to return from embedding search"
    )
    chunk_size: int = Field(default=1200, description="")
    chunk_overlap: int = Field(default=240, description="")
    sentence_level: bool = Field(
        default=True, description="Use sentence level filtering or not"
    )
    ...
    parallel_sections: bool = Field(
        default=False, description="Whether to process sections in parallel"
    )
    ...
    section_agent_react: bool = Field(
        default=True, description="Whether to use ReAct agent for section processing"
    )
    use_agraph: bool = Field(
        default=False, description="Whether to use AGraph for knowledge retrieval"
    )
