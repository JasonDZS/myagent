...
class Run(BaseModel):
    ...
    parent_run_id: str | None = Field(
        None, description="Parent run identifier for nested runs"
    )
    ...
    end_time: datetime | None = Field(None, description="End time when completed")
    ...
    stacktrace: str | None = Field(None, description="Full error stacktrace")
    ...
    token_usage: dict[str, int] | None = Field(
        None, description="Token usage for LLM calls"
    )
    ...
    def end(self, outputs: dict[str, Any] | None = None) -> "Run":
    ...
    def fail(
        self, error: str, error_type: str | None = None, stacktrace: str | None = None
    ) -> "Run":
...
class Trace(BaseModel):
    ...
    # Core identifiers
    id: str = Field(
        default_factory=lambda: str(uuid.uuid4()), description="Unique trace identifier"
    )
    ...
    # Request context
    request: str | None = Field(None, description="Original user request")
    response: str | None = Field(None, description="Final response")

    # Runs
    runs: list[Run] = Field(
        default_factory=list, description="List of runs in this trace"
    )

    # Metadata
    metadata: TraceMetadata = Field(
        default_factory=TraceMetadata, description="Trace metadata"
    )
    status: RunStatus = Field(
        default=RunStatus.PENDING, description="Overall trace status"
    )
    ...
    def end(self, response: str | None = None) -> "Trace":
