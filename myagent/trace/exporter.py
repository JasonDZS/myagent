...
class TraceExporter:
    ...
    async def export_traces_to_json(
        ...
        json_data = json.dumps(
            export_data, indent=2, default=str, ensure_ascii=ensure_ascii
        )
    ...
    async def export_traces_to_csv(
        self, filters: TraceFilter | None = None, file_path: str | Path | None = None
    ) -> str:
    ...
    async def export_runs_to_csv(
        self, filters: RunFilter | None = None, file_path: str | Path | None = None
    ) -> str:
    ...
    def export_trace_tree(self, trace: Trace) -> str:
        ...
        def add_run_to_tree(run: Run, prefix: str = "", is_last: bool = True):
            ...
            lines.append(
                f"{prefix}{connector}{status_icon} {run.name} ({run.run_type.value}) - {duration}"
            )
