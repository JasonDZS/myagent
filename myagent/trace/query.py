...
from .models import Trace, Run, RunType, RunStatus
...
class TraceFilter(BaseModel):
    ...
    # Time range
    start_time: Optional[datetime] = Field(None, description="Filter traces after this time")
    end_time: Optional[datetime] = Field(None, description="Filter traces before this time")
    
    # Status
    status: Optional[RunStatus] = Field(None, description="Filter by trace status")
    ...
    # Content filters
    request_contains: Optional[str] = Field(None, description="Filter by request content")
    response_contains: Optional[str] = Field(None, description="Filter by response content")
    ...
    min_tokens: Optional[int] = Field(None, description="Minimum token count")
    max_tokens: Optional[int] = Field(None, description="Maximum token count")
    
    # Error filters
    has_errors: Optional[bool] = Field(None, description="Filter traces with/without errors")
...
class RunFilter(BaseModel):
    ...
    status: Optional[RunStatus] = Field(None, description="Filter by run status")
    name_contains: Optional[str] = Field(None, description="Filter by run name")
    
    # Time range
    start_time: Optional[datetime] = Field(None, description="Filter runs after this time")
    end_time: Optional[datetime] = Field(None, description="Filter runs before this time")
    ...
    # Hierarchy filters
    is_root: Optional[bool] = Field(None, description="Filter root runs (no parent)")
    parent_run_id: Optional[str] = Field(None, description="Filter by parent run ID")
...
class QueryOptions(BaseModel):
    ...
    sort_by: List[SortCriteria] = Field(default_factory=lambda: [SortCriteria(field="start_time")])
    include_runs: bool = Field(default=True, description="Include runs in trace results")
...
class QueryResult(BaseModel):
    ...
    results: List[Union[Trace, Run]] = Field(..., description="Query results")
...
class TraceQueryEngine:
    ...
    def _filter_traces(self, traces: List[Trace], filters: TraceFilter) -> List[Trace]:
        ...
        if filters.session_id:
            filtered = [t for t in filtered if t.metadata.session_id == filters.session_id]
        if filters.environment:
            filtered = [t for t in filtered if t.metadata.environment == filters.environment]
        if filters.tags:
            filtered = [t for t in filtered if any(tag in t.metadata.tags for tag in filters.tags)]
        
        # Content filters
        if filters.request_contains:
            filtered = [t for t in filtered if t.request and filters.request_contains.lower() in t.request.lower()]
        if filters.response_contains:
            filtered = [t for t in filtered if t.response and filters.response_contains.lower() in t.response.lower()]
        
        # Metrics filters
        if filters.min_duration_ms is not None:
            filtered = [t for t in filtered if t.duration_ms and t.duration_ms >= filters.min_duration_ms]
        if filters.max_duration_ms is not None:
            filtered = [t for t in filtered if t.duration_ms and t.duration_ms <= filters.max_duration_ms]
        if filters.min_cost is not None:
            filtered = [t for t in filtered if t.total_cost and t.total_cost >= filters.min_cost]
        if filters.max_cost is not None:
            filtered = [t for t in filtered if t.total_cost and t.total_cost <= filters.max_cost]
        if filters.min_tokens is not None:
            filtered = [t for t in filtered if t.total_tokens and t.total_tokens >= filters.min_tokens]
        if filters.max_tokens is not None:
            filtered = [t for t in filtered if t.total_tokens and t.total_tokens <= filters.max_tokens]
        
        # Error filters
        if filters.has_errors is not None:
            if filters.has_errors:
                filtered = [t for t in filtered if any(run.status == RunStatus.ERROR for run in t.runs)]
            else:
                filtered = [t for t in filtered if not any(run.status == RunStatus.ERROR for run in t.runs)]
    ...
    def _filter_runs(self, runs: List[Run], filters: RunFilter) -> List[Run]:
        ...
        if filters.name_contains:
            filtered = [r for r in filtered if filters.name_contains.lower() in r.name.lower()]
        ...
        # Metrics filters
        if filters.min_duration_ms is not None:
            filtered = [r for r in filtered if r.duration_ms and r.duration_ms >= filters.min_duration_ms]
        if filters.max_duration_ms is not None:
            filtered = [r for r in filtered if r.duration_ms and r.duration_ms <= filters.max_duration_ms]
    ...
    def _sort_traces(self, traces: List[Trace], sort_criteria: List[SortCriteria]) -> List[Trace]:
        """Sort traces by multiple criteria."""
        for criteria in reversed(sort_criteria):  # Apply in reverse order for stable sort
    ...
    def _sort_runs(self, runs: List[Run], sort_criteria: List[SortCriteria]) -> List[Run]:
    ...
    async def get_trace_statistics(self, filters: Optional[TraceFilter] = None) -> Dict[str, Any]:
    ...
    async def get_run_statistics(self, filters: Optional[RunFilter] = None) -> Dict[str, Any]:
